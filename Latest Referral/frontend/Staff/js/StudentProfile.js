// Counselor/Staff StudentProfile.js - Fixed Version
document.addEventListener('DOMContentLoaded', async function() {
  ...
  function filterAdvisers() {
    ...
    const filtered = allAdvisers.filter(adviser => {
      return adviser.fullName.toLowerCase().includes(search) ||
             (adviser.email && adviser.email.toLowerCase().includes(search)) ||
             (adviser.department && adviser.department.toLowerCase().includes(search));
    });
  ...
  // Event listener for adviser search
  ...
  modalLevelFilter.addEventListener('change', filterModalStudents);
  modalGradeFilter.addEventListener('change', filterModalStudents);

  // Load all students (for student view) - FIXED VERSION
  ...
  studentViewBtn.addEventListener('click', async () => {
    ...
    // Always reload students when switching to this view
