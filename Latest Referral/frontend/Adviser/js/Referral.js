// Teacher/Adviser Referral.js - WITH STUDENT AUTOCOMPLETE
...
// Store students data for autocomplete
let studentsData = [];
...
async function initializePage() {
  try {
    ...
    await loadStudentsForAutocomplete();
...
async function loadStudentsForAutocomplete() {
  try {
    console.log('📚 Loading students for autocomplete...');
    
    const response = await apiClient.get('/students');
    
    if (response.success) {
      studentsData = response.data || [];
      console.log('✅ Loaded', studentsData.length, 'students for autocomplete');
    } else {
...
function setupStudentAutocomplete() {
  ...
  // Insert after the student ID input's parent
  ...
  studentIdInput.addEventListener('input', function() {
    ...
    // Display matches
    autocompleteContainer.innerHTML = matches.slice(0, 5).map(student => {
      ...
      return `
        ...
        " data-student='${JSON.stringify(student)}'>
          ...
          <div style="font-size: 0.9rem; color: #e0e0e0; margin-bottom: 2px;">
            ${fullName}
          </div>
    ...
    items.forEach(item => {
      ...
      item.addEventListener('click', function() {
        ...
        // Set level value first
        if (levelSelect && student.level) {
          levelSelect.value = student.level;
          console.log('Level set to:', student.level);
        }
        
        // Trigger level change to update grade options
        if (student.level && gradeSelect) {
          updateGradeOptions(student.level, gradeSelect);
          
          // Use longer delay and log the options
          setTimeout(() => {
            console.log('Grade options:', Array.from(gradeSelect.options).map(opt => opt.value));
            console.log('Trying to set grade to:', student.grade);
            
            if (student.grade) {
              // Normalize the grade format
              // Student grade might be "12" or "Grade 12"
              // Dropdown options are "Grade 1", "Grade 2", etc.
              let normalizedGrade = student.grade;
              
              // If the grade doesn't start with "Grade", add it
              ...
              gradeSelect.value = normalizedGrade;
              console.log('Grade select value after setting:', gradeSelect.value);
              
              // If still not set, try without "Grade" prefix
              if (!gradeSelect.value || gradeSelect.value === '') {
                const gradeNumber = student.grade.replace('Grade ', '').trim();
                gradeSelect.value = gradeNumber;
                console.log('Tried grade number:', gradeNumber, 'Result:', gradeSelect.value);
              }
              
              // If still not set, try to find a matching option manually
              if (!gradeSelect.value || gradeSelect.value === '') {
                const options = Array.from(gradeSelect.options);
                const matchingOption = options.find(opt => 
                  opt.value === student.grade || 
                  opt.value === normalizedGrade ||
                  opt.textContent === student.grade ||
                  opt.textContent === normalizedGrade
                );
                if (matchingOption) {
                  gradeSelect.value = matchingOption.value;
                  console.log('Found matching option, set to:', matchingOption.value);
                } else {
                  console.warn('No matching grade option found for:', student.grade);
                }
              }
              
              // Final success check
              if (gradeSelect.value && gradeSelect.value !== '') {
                console.log('✅ Grade successfully set to:', gradeSelect.value);
              }
            }
          }, 100);
        }
  ...
  document.addEventListener('click', function(e) {
    if (!studentIdInput.contains(e.target) && !autocompleteContainer.contains(e.target)) {
      autocompleteContainer.style.display = 'none';
    }
  });
...
function openAddModal() {
  ...
  if (modal) {
    ...
    if (form) {
      form.reset();
      // No need to set default severity anymore - it's removed
    }
...
function closeAddModal() {
  ...
  const autocompleteContainer = document.querySelector('.autocomplete-dropdown');
...
function displayReferrals(referrals) {
  ...
  tbody.innerHTML = referrals.map(referral => {
    ...
    // Display severity with special handling for pending status
    let severityDisplay = referral.severity || 'Pending Assessment';
    let severityClass = getSeverityClass(referral.severity);
    
    // If status is Pending and no severity set, show "Pending Assessment"
    if (referral.status === 'Pending' && (!referral.severity || referral.severity === 'Pending Assessment')) {
      severityDisplay = 'Pending Assessment';
      severityClass = 'severity-pending';
    }
...
function getSeverityClass(severity) {
  const severityMap = {
    ...
    'High': 'severity-high',
    'Pending Assessment': 'severity-pending'
  };
  return severityMap[severity] || 'severity-pending';
}
...
// Handle add referral - NO CATEGORY OR SEVERITY FOR TEACHERS
async function handleAddReferral(e) {
  ...
  const data = {
    ...
    reason,
    description
    // NOTE: Severity will be set to "Pending Assessment" by default
    // Only counselors/staff can set actual severity during review
  };
